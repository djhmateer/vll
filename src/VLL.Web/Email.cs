...
namespace VLL.Web
{
    ...
    {
        public static async Task<bool> SendTemplate(string templateName, string toEmailAddress, string dataToSendUser,
            string postmarkServerToken)
        {
            ...
            else if (templateName == "forgot-password")
            {
                // html
                var htmlMiddle =
                    await File.ReadAllTextAsync(Path.Combine(folder, "html-template-forgot-password.html"));
                htmlMiddle = htmlMiddle.Replace("{{guid}}", dataToSendUser);
                htmlBody = htmlTop + htmlMiddle + htmlBottom;
            ...
            else if (templateName == "face-search-job-complete")
            {
                // html
                var htmlMiddle =
                    await File.ReadAllTextAsync(Path.Combine(folder, "html-template-face-search-job-complete.html"));
            ...
            else if (templateName == "hate-speech-job-complete")
            {
                // html
                var htmlMiddle =
                    await File.ReadAllTextAsync(Path.Combine(folder, "html-template-hate-speech-job-complete.html"));
                htmlMiddle = htmlMiddle.Replace("{{jobId}}", dataToSendUser);
                htmlBody = htmlTop + htmlMiddle + htmlBottom;
            ...
            else if (templateName == "speech-parts-job-complete")
            {
                // html
                var htmlMiddle =
                    await File.ReadAllTextAsync(Path.Combine(folder, "html-template-speech-parts-job-complete.html"));
                htmlMiddle = htmlMiddle.Replace("{{jobId}}", dataToSendUser);
                htmlBody = htmlTop + htmlMiddle + htmlBottom;
        ...
        public static async Task<bool> Send(OSREmail osrEmail, string postmarkServerToken)
        {
            ...
            if (string.IsNullOrWhiteSpace(postmarkServerToken))
                throw new ArgumentNullException(nameof(postmarkServerToken));
                  ...
                  <table border=""0"" cellpadding=""0"" cellspacing=""0"" style=""border-collapse: separate; mso-table-lspace: 0pt; mso-table-rspace: 0pt; width: 100%;"">
                    <tr>
                      <td style=""font-family: sans-serif; font-size: 14px; vertical-align: top;"">
                        ...
                        <p style=""font-family: sans-serif; font-size: 14px; font-weight: normal; margin: 0; Margin-bottom: 15px;"">auto-archiver team.</p>
